...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            watch(showExplanationImage, async (newVal) => {
                if (newVal && !explanationDropzone.value) {
                    await nextTick()

                    if (Dropzone.instances.some(dz => dz.element.id === 'explanation-dropzone')) {
                        return
                    }

                    explanationDropzone.value = new Dropzone('#explanation-dropzone', {
                        url: '/',
