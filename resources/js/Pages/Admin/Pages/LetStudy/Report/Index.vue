<template>
    ...
    <Head>
        <title>Section Impact Report − LetStudy</title>
    </Head>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        <ol class="breadcrumb text-muted fs-6 fw-semibold mb-3">
                            ...
                            <li class="breadcrumb-item pe-3 text-muted">Impact Report</li>
                        </ol>
                        <div class="page-heading">
                            <h1 class="mb-0 text-dark">Section Impact Report</h1>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="row align-items-start m-0">
                                <div class="col-12 col-md-4 col-xl-3 p-0">
                                    <div class="card-header p-6 d-flex flex-column m-0 border-0" style="min-height: unset;">
                                        <h3 class="mb-0">Impact Report</h3>
                                        <p class="fs-5 d-none d-md-block text-gray-600 mt-2 mb-0 fw-normal">
                                            Mengelola informasi yang tampil di section impact report 
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="col-12 col-md-8 col-xl-9 card-body p-6">
                                    <div class="mb-5 fv-row">
                                        <label class="form-label fs-6">Sampul Laporan </label>
                                        <div id="report-dropzone"
                                    ...
                                    <div class="fv-row">
                                        <label class="required form-label fs-6">ID Laporan</label>
                                        <input type="text" class="form-control fs-5" v-model="form.id_report"
                                            required placeholder="Masukkan ID google drive disini..."
                                            :class="{ 'is-invalid': errors.id_report }" />
                                        <div v-if="errors.id_report" class="text-mydanger mt-2">
                                            {{ errors . id_report }}
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            const form = reactive({
                heading: props.report.heading,
                sub_heading: props.report.sub_heading,
                id_report: props.report.id_report,
                image: null,
            });

            onMounted(() => {
                const dz = new Dropzone('#report-dropzone', {
                    url: '/', 
                    paramName: 'image',
                    maxFiles: 1,
                    maxFilesize: 10,
                    ...
                    init: function () {
                        ...
                        if (props.report.image) {
                            const mockFile = {
                                name: props.report.image_name ?? 'Impact Report',
                                size: props.report.image_size ?? 12345
                            };
                            this.emit('addedfile', mockFile);
                            this.emit('thumbnail', mockFile, `/storage/${props.report.image}`);
            ...
            const submit = () => {
                ...
                payload.append('id_report', form.id_report);
                ...
                router.post(`/admin/pages/letstudy/report/${props.report.id}`, payload, {
                    onSuccess: () => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
