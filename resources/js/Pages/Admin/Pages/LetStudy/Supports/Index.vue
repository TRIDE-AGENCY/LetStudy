<template>
    ...
    <Head>
        <title>Section Dukungan − LetStudy</title>
    </Head>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        <ol class="breadcrumb text-muted fs-6 fw-semibold mb-3">
                            ...
                            <li class="breadcrumb-item pe-3 text-muted">Dukungan</li>
                        </ol>
                        <div class="page-heading">
                            <h1 class="mb-0">Section Dukungan</h1>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div id="kt_app_content_container" class="app-container container-xxl px-0">
                    <div class="card rounded-4 border border-gray-300">
                            ...
                            <div class="flex-grow-1 ">
                                <form @submit.prevent="handleSearch"
                                    ...
                                    <input type="text" class="form-control form-control-solid ps-13 fs-5"
                                        placeholder="Cari pendukung disini..." v-model="search" />
                                </form>
                            </div>
                        ...
                        <div class="card-body p-0">
                            <div class="text-center border-top border-bottom border-gray-300 py-10 px-6" v-if="supports.data.length === 0">
                                <img src="/assets/media/illustrations/empty.png"
                                    alt="Tidak ada data" class="mh-225px img-fluid mb-10">
                                <h2 class="text-dark mb-4">Pendukung Tidak Ditemukan</h2>
                                <p class="text-gray-600 fs-4 mb-0">Belum ada pendukung atau pencarian tidak sesuai.</p>
                            </div>
                                ...
                                <table id="kt_datatable_horizontal_scroll"
                                    ...
                                    <thead>
                                        <tr class="text-start text-gray-600 fw-bold fs-7 text-uppercase gs-0">
                                            ...
                                            <th class="bg-secondary min-w-150px" style="letter-spacing: 0.04em;">Logo</th>
                                            <th class="bg-secondary min-w-150px" style="letter-spacing: 0.04em;">Nama</th>
                                            <th class="bg-secondary min-w-200px" style="letter-spacing: 0.04em;">Link</th>
                                            <th class="bg-secondary pe-6 min-w-125px" style="letter-spacing: 0.04em;"></th>
                                        </tr>
                                    </thead>
                                    <tbody class="fw-semibold text-gray-600">
                                        <tr v-for="(support, index) in supports.data" :key="index">
                                            <td class="ps-6">
                                                {{ ++index + (supports . current_page - 1) * supports . per_page }}
                                            </td>
                                            <td>
                                                <img class="h-30px" :src="`/storage/${support.logo}`" :alt="`Logo ${support.name}`">
                                            </td>
                                            <td class="text-dark fw-bold">{{ support . name }}</td>
                                            <td class="mw-200px text-truncate">
                                                <a :href="support.link" target="_blank" class="btn-link-myprimary" >
                                                    {{ support . link }}
                                                </a>
                                            </td>
                                            <td class="text-end pe-6">
                                                <button @click="editSupport(support)"
                                                    class="btn btn-sm btn-icon btn-mysecondary me-2">
                                                    <i class="ri-edit-2-fill fs-3"></i></button>
                                                <button @click.prevent="destroy(support.id)"
                            ...
                            <Pagination :links="supports.links" align="end" class="py-5 px-6" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card w-100 rounded-0">
                    <div class="card-header ps-6 pe-3 h-60px border-bottom border-gray-300 h-60px h-lg-70px">
                        <div class="card-title">
                            <h2>{{ isEditMode ? 'Edit Pendukung' : 'Tambah Pendukung' }}</h2>
                        </div>
                            ...
                            <div class="card rounded-4 border border-gray-300">
                                ...
                                <div class="card-body p-6">
                                    <div class="mb-5 fv-row">
                                        ...
                                        <div id="support-dropzone" :key="dropzoneKey"
                                    ...
                                    <div class="fv-row">
                                        <label class="required form-label fs-6">Link Website / Sosmed</label>
                                        <input type="text" class="form-control fs-5" v-model="form.link"
                                            required placeholder="Masukkan link website / sosmed disini..."
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="submit" id="kt_drawer_submit_button"
                                ...
                                <span class="indicator-label">
                                    {{ isEditMode ? 'Edit Pendukung' : 'Tambah Pendukung' }}
                                </span>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            const handleSearch = () => {
                router.get('/admin/pages/letstudy/supports', {
                    q: search.value,
                });
            }
            ...
            const submit = () => {
                ...
                if (isEditMode.value && editing.value) {
                    ...
                    router.post(`/admin/pages/letstudy/supports/${editing.value.id}`, payload, {
                        onSuccess: () => {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
                                text: 'Data Pendukung Berhasil Diperbarui!',
                                timer: 2000,
                    ...
                    router.post('/admin/pages/letstudy/supports', payload, {
                        onSuccess: () => {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
            ...
            const editSupport = (support) => {
                ...
                editing.value = support;

                form.name = support.name;
                form.logo = null;
                form.link = support.link;
            ...
            const destroy = (id) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        router.delete(`/admin/pages/letstudy/supports/${id}`);
                        Swal.fire({
                            icon: 'success',
                            title: 'Dihapus!',
                            text: 'Data Pendukung Berhasil Dihapus!',
                            timer: 2000,
                            timerProgressBar: true,
                            showConfirmButton: false,
                            customClass: {
            ...
            watch(dropzoneKey, () => {
                nextTick(() => {
                    const dz = new Dropzone('#support-dropzone', {
                        url: '/',
                        paramName: 'logo',
                        maxFiles: 1,
                        maxFilesize: 10,
